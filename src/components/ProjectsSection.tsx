      ...
      >
        <div className="p-3 flex gap-2">
          ...
          <div>
            ...
            <p className="text-white/30 text-[6px] mt-0.5">Creating stories that resonate</p>
          </div>
        </div>
      </div>
    </div>
...
function LuminMockup() {
  return (
    <div className="relative w-full h-full flex items-center justify-center p-6">
      <div
        className="w-full max-w-[90%] rounded-xl overflow-hidden shadow-2xl"
      ...
      >
        ...
        <div
          ...
          style={{ background: "linear-gradient(135deg, #0a0a0a 0%, #111 100%)", minHeight: "160px" }}
        >
      ...
      <div className="absolute bottom-10 right-14 w-8 h-8 rounded-full border border-[#6EFF6E]/60 flex items-center justify-center">
        <span style={{ color: "#6EFF6E", fontSize: "12px" }}>↗</span>
      </div>
    </div>
  );
}
  ...
  return (
    ...
    >
      <a
        ...
        style={{ background: project.bgColor, height: "340px", transition: "transform 0.4s ease" }}
      ...
      <div className="flex flex-wrap gap-2">
        {project.tags.map((tag) => (
          <span key={tag} className="text-xs px-3 py-1 rounded-full border border-white/15 text-white/60">
            {tag}
          </span>
        ))}
      </div>
...
export default function ProjectsSection({ menuOpen, onToggleMenu }: ProjectsSectionProps) {
  const headerRef = useRef<HTMLDivElement>(null);
  const headerInView = useInView(headerRef, { once: true });

  return (
    <section id="project" className="relative" style={{ background: "#1c1c1c" }}>

      {/* Sticky navbar */}
      <div className="sticky top-0 z-30" style={{ background: "#1c1c1c" }}>
        <Navbar label="// Explore Work" menuOpen={menuOpen} onToggleMenu={onToggleMenu} />
      </div>
      ...
      <div className="px-8 pt-4 pb-24">
        <div ref={headerRef}>
          <motion.h2
            ...
            className="text-white font-bold leading-tight max-w-sm mb-3"
            style={{ fontFamily: "'DM Sans', sans-serif", fontSize: "clamp(28px, 4vw, 46px)" }}
          >
